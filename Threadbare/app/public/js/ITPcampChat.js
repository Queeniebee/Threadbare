// Threadbare Chat client-side functionality
;(function($, window, io) {
	...
	var chatClient = {
		...
		'fn': {
			...
			'userReady': function( data ) {
				...
				chatClient.cfg.$incoming
					.append( '<div class="hot-green"> > *** Welcome to ITP CAMP CHAT '+decoded+'!!! ***</div>')
					.append( '<div class="hot-green"> > ***  '+countStr+' ***</div>');
				chatClient.fn.autoscroll();
			},
			...
			'userDisconnected': function( data ) {
				if( data.name ) {
					var decoded = decodeURIComponent( data.name );
					chatClient.cfg.$incoming
						.append('<span style="color:#095914;font-style:italic;"> > '+decoded+' disconnected</span><br>');
					chatClient.fn.autoscroll();
				}
			}
		}
